...
def dmbcg(dataset, score_type, parents_max, parents_number):
	...
	for i in range(len(initial_order)):
	  ...
	  # Ordena os dados com base na solução
		...
		#print(f'\n imprimindo o grafo DMBC da variavel: {i}' )
		 ...
		 #   "font_size": 10,
		  #  "node_size": 1000,
		   # "node_color": "white",
			...
			#"width": 1,
		#}
		#nx.draw_networkx(localGraph, pos, **options)
...
#		ax = plt.gca()
 #	   ax.margins(0.20)
  #	  plt.axis("off")
   #	 plt.show()
		#fileName = "drive/MyDrive/Colab Notebooks/BaseDados-1000/grafo" + str(i) + ".jpg"
	...
	#for node in finalGraph.nodes:
	 #   for neighbor in finalGraph.neighbors(node):
	  #	  if finalGraph.has_edge(node, neighbor) and finalGraph.has_edge(neighbor, node):
	   #		 if finalGraph.get_edge_data(node, neighbor)['weight'] < finalGraph.get_edge_data(neighbor,node)['weight']:
		#			edges_to_remove.append((node, neighbor)) # Add edge to removal list
				#except KeyError:
					#print(f"Edge ({node}, {neighbor}) or ({neighbor}, {node}) not found. Skipping...")

	# Remove algumas arestas do grafo de acordo com peso
	edges_to_remove = []
	for node in finalGraph.nodes:
		for neighbor in finalGraph.neighbors(node):
			if finalGraph.get_edge_data(node, neighbor)['weight'] < 3: #len(nodes)/3:
				edges_to_remove.append((node, neighbor)) # Add edge to removal list
